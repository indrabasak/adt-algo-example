 ...
 * of length 1. The number 15 has binary representation 1111 and has no
...
class BinaryGap {
    ...
    public int solution(int N) {
        ...
        char[] binaryDigits = binaryStr.toCharArray();
        ...
        for (char digit : binaryDigits) {
            if (digit == '1') {
                if (seenOne && (binaryGap > highestBinaryGap)) {
                        highestBinaryGap = binaryGap;
                }
                binaryGap = 0;
                seenOne = true;
            } else {
